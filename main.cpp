...
void add(MyVector<string> &theVector, const string &jobTitle, int timeSlot) {
  if (timeSlot >= theVector.end())
    theVector.insert(theVector.end(), jobTitle);
  else
    theVector.insert(timeSlot, jobTitle);
}
...
void fire(MyVector<string> &theVector, const string &jobTitle) {
  theVector.erase(theVector.find(jobTitle));
}

// If will is fired from a job in a specific time slot
// it deletes that time slot from his schedule
void fire(MyVector<string> &theVector, int timeSlot) {
  theVector.erase(timeSlot);
}
...
// Perform the "Car crash" events
void carCrash(MyVector<string> &theVector, int timeSlot) {
  for (int iter = timeSlot; iter < theVector.end(); iter++) {
    theVector.erase(iter);
  }
}

// Perform the "Disease" event
void disease(MyVector<string> &theVector) { theVector.clear(); }

// Perform the "Guilt" event
void guilt(MyVector<string> &theVector) { theVector.shrink_to_fit(); }
